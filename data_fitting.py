...
def fit(model, init_bundle, betas, gammas, lams, deltas, known_points, steps, writer, epochs=100, lr=8e-4,
        loss_mode='mse',
        infected_weight=1.,
        recovered_weight=1., deaths_weight=1., force_init=False, verbose=False):
    model.eval()
    ...
    d_0 = uniform(init_bundle[4][0], init_bundle[4][1], size=1)
    ...
    beta, gamma, lam, delta = torch.Tensor([beta]).reshape(-1, 1), torch.Tensor([gamma]).reshape(-1, 1), torch.Tensor([lam]).reshape(-1, 1), torch.Tensor([delta]).reshape(-1, 1)
    ...
    if force_init:
        ...
        r_0 = torch.Tensor([r_0]).reshape(-1, 1)
        d_0 = torch.Tensor([d_0]).reshape(-1, 1)
        optimizer = torch.optim.SGD([e_0, i_0, r_0, d_0, beta, gamma, lam, delta], lr=lr)

    s_0 = 1 - (e_0 + i_0 + r_0 + d_0)

    rnd_init = [round(e_0.item(), 5), round(i_0.item(), 5), round(r_0.item(), 5), round(d_0.item(), 5),
                round(beta.item(), 5),
                round(gamma.item(), 5), round(lam.item(), 5), round(delta.item(), 5)]
    ...
    for epoch in tqdm(range(epochs), desc='Finding the best inputs', disable=not verbose):
        ...
        for t in known_t:
            ...
            s_hat, e_hat, i_hat, r_hat, d_hat = model.parametric_solution(t_tensor, initial_conditions, beta=beta,
                                                                          gamma=gamma,
                                                                          lam=lam, delta=delta)
            ...
            loss_d = loss_d * deaths_weight

            loss += loss_i + loss_r + loss_d + regularization
    ...
    return e_0, i_0, r_0, d_0, beta, gamma, lam, delta, rnd_init, losses[-1]
